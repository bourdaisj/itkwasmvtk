...
export async function convertToPolyData(files: File[]) {
  ...
  const args = [file.name, file.name + "_output.vtp"];
...
export async function convertEnsightToPolyData(files: File[]) {
  ...
  const args = [caseFile.name, caseFile.name + "_output.vtp"];
...
export async function convertUGToPolyData(files: File[]) {
  console.debug("convertUGToPolyData", files);
  const file = files[0];
